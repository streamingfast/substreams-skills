...
**`ethabi` is no longer needed.** Abigen used to write bare `ethabi::ParamType` / `ethabi::Token` paths into the file it generates in your crate, which forced you to declare `ethabi` yourself. Since 0.12 the generator emits its own decoding and references only `substreams_ethereum`, so no `ethabi` dependency is required on either path.

Do not add one back: `ethabi` pulls `ethereum-types` → `rand` → `getrandom`, and `getrandom` fails to build for `wasm32-unknown-unknown` with *"the wasm*-unknown-unknown targets are not supported by default"*. If you inherited an `ethabi` dependency, regenerate the bindings and drop it.

**`getrandom` and `init!()` are not required.** Older docs say to add a `getrandom` dependency and call `substreams_ethereum::init!();` — that guidance is stale. 0.12 declares no `getrandom` at all (it came in through `ethabi`, now gone), and `init!()` expands to nothing, kept only so existing code keeps compiling. Declaring `getrandom` yourself is what *causes* the wasm32 build error, so leave both out.
...
## Protobuf codegen (buf.gen.yaml)

Domain protobufs are generated by `buf generate`, not from `build.rs`. Generated code is **checked in**.
...
`lazy_views=true` is what produces the `*LazyView` types; `unknown_fields=false` drops round-trip fidelity you do not need in WASM and keeps decode cheap. Add `**/.last_generated_hash` to `.gitignore` — `buf` writes it and does not ignore it for you.
...
| `getrandom` "not supported by default" on wasm32 | You pinned a stray `getrandom` **without** `features = ["custom"]`, or an old `substreams-ethereum`. Current versions supply this themselves — remove your own `getrandom` dep rather than adding one |
| `failed to resolve: use of undeclared crate 'ethabi'` in `src/abi/*.rs` | Stale bindings from an older Abigen. Regenerate: 0.12 emits no `ethabi` reference at all |
| `getrandom` "not supported by default" from an `ethabi` dependency | `ethabi` pulls `ethereum-types` → `rand` → `getrandom`, which fails on wasm32. Drop `ethabi`; 0.12 does not need it |

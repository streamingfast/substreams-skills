...
- `portal-api-jwt` (v1.5.2): correct the device-code flow against the live API — the `DeviceAuthorize` example showed snake_case and camelCase keys in one object (a response that never occurs; the wire format is camelCase only), and typed `expiresIn`/`interval` as numbers when proto3 emits `int64` as strings; document that `SLOW_DOWN` rate-limits per device code at one call per `interval` and *masks* the true status (an approved login can read `SLOW_DOWN`, so it must never be read as "not approved"); document that errors are HTTP 4xx Connect bodies rather than a `status` enum (a never-issued device code → 400 `invalid_argument`, any `RefreshToken` failure → 401 `unauthenticated`, indistinguishable, so never retry), while an aged-out code stays a 200 `DEVICE_TOKEN_STATUS_EXPIRED`.

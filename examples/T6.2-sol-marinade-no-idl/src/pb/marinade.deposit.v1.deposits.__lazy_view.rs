...
/** Lazy view of `marinade.deposit.v1.Deposit`: nested and repeated message fields are
...
/** Lazy view of `marinade.deposit.v1.Deposits`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `DepositsView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `DepositsView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = DepositsLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct DepositsLazyView<'a> {
    /// Field 1: `deposits`
...
impl<'a> ::buffa::LazyMessageView<'a> for DepositsLazyView<'a> {
...
impl<'a> ::buffa::MessageName for DepositsLazyView<'a> {
    const PACKAGE: &'static str = "marinade.deposit.v1";

...
- All 13 buildable examples, `substreams-dev`, `substreams-ethereum` (and `abi-codegen.md`), `substreams-convert` (`subgraph.md` / `solana-contract.md`), `substreams-sql`, `substreams-sink`: move the documented stack to the published buffa SDKs — `substreams 0.8.0-beta`, `substreams-ethereum 0.12.0-beta.1`, `substreams-solana 0.16.0-beta.1`, `substreams-database-change 5.0.0-beta.1`. The `substreams-convert` references still carried a whole prost-era `Cargo.toml` (`substreams 0.7`, `prost`, `prost-build`), and six places still pinned `substreams-database-change = "4"`, which no longer compiles against the buffa stack (`DatabaseChanges: Message` is not satisfied).

- **Reverses the `ethabi` guidance from v1.0.2.** 0.12's Abigen emits no `ethabi` reference at all, so it is no longer a required dependency on the Abigen path — and adding it now *breaks* the build: `ethabi` pulls `ethereum-types` → `rand` → `getrandom`, which refuses to compile for `wasm32-unknown-unknown`. Three examples (T2.2, T3.1, T3.2) declared `ethabi = "18"` and were failing to build; removing it let their bindings regenerate ethabi-free, 20% smaller. `getrandom` and `init!()` stay unnecessary, but for a new reason: 0.12 declares no `getrandom` itself, and `init!()` now expands to nothing.
...
- `substreams-convert` (v1.3.1): review fixes against sibling skills — remove SQL Delta auto-CREATE myth (`create_row` / `update_row` / `upsert_row` + `.set`, no `set_bigdecimal`); rewrite sink recipe (`setup` then `run <dsn> <manifest|spkg> [range]`, fix `mismatch` typo, no double-scheme DSN, module from `sink:` only); complete SQL manifest includes standard `imports:` (database-changes + sql protodefs) and `sink:`; `ethereum_common` uses full `https://spkg.io/v1/packages/ethereum-common/v0.3.3` URL with param-override warning; temper §5 `graph_out` to optional/legacy + entity-change crate caveat (SQL-first greenfield); Solana drops dead `Block as BlockExt`, uses `meta.as_ref()`, and stores amounts as `bigint` not `u64 as i64`; add pre-flight (source → network → destination) and specialist-skill handoffs; ethereum loop prefers `logs_with_calls`.

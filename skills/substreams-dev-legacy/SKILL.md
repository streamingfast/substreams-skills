---
name: substreams-dev-legacy
description: Substreams development on the pre-buffa stack — substreams 0.7.x and below, where protobuf is prost. Use when the project pins substreams <= 0.7, substreams-ethereum <= 0.11, substreams-solana <= 0.15, or substreams-database-change <= 4, or when Cargo.toml declares prost / prost-build. On substreams 0.8+ use substreams-dev instead; the two stacks generate incompatible types.
license: Apache-2.0
compatibility:
...
# Substreams Development Expert (legacy, prost)

> **Check the version before using this skill.** It documents `substreams` **0.7.x and below**, where
> protobuf is [prost](https://github.com/tokio-rs/prost). `substreams` 0.8.0 replaced prost with
> [buffa](https://github.com/anthropics/buffa) and the generated types are not interchangeable.
>
> | `substreams` | Protobuf | Skill |
> |---|---|---|
> | 0.8.0+ | buffa | **`substreams-dev`** |
> | 0.7.x and below | prost | **this skill** |
>
> Read the user's `Cargo.toml` first. On 0.8+, stop and load `substreams-dev` — the codegen,
> field access and handler signatures documented here are all wrong for that stack.
>
> This skill is frozen at the last pre-buffa revision and receives fixes only for prost-era bugs.
> The chain skills (`substreams-ethereum`, `substreams-solana`) and sink skills document the buffa
> stack only; on 0.7 treat their crate versions and protobuf idioms as inapplicable, and pin
> `substreams-ethereum` 0.11 / `substreams-solana` 0.15 / `substreams-database-change` 4.

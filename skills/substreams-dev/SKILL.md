...
| Project pins `substreams` **0.7.x or below** (prost, not buffa) | **`substreams-dev-legacy`** |
...
## Versions — buffa or prost

**`substreams` 0.8.0 replaced `prost` with [buffa](https://github.com/anthropics/buffa) as the
protobuf implementation.** This is the single most important thing to establish before writing any
code, because the two generate incompatible types and cannot be mixed in one dependency tree.

**Check the user's `Cargo.toml` first.** Everything below — codegen, field access, handler
signatures — follows from which row they are on.

| `substreams` | Protobuf | EVM | Solana | SQL sink |
|---|---|---|---|---|
| **0.8.0-beta +** | buffa | `substreams-ethereum` 0.12+ | `substreams-solana` 0.16+ | `substreams-database-change` 5+ |
| 0.7.x and below | prost | `substreams-ethereum` 0.11 | `substreams-solana` 0.15 | `substreams-database-change` 4 |

**Never mix rows.** A prost-generated `substreams-ethereum` against `substreams` 0.8 does not
compile; worse, some mismatches link successfully and pull two `substreams` copies into the binary.
`cargo tree | grep substreams` should show exactly one version.

### If the user is on 0.7.x or below

This skill documents the buffa stack. On the prost stack these differ:
...
| Lazy views | not available | `lazy_views=true`, `&FooLazyView<'_>` handlers |

**Load `substreams-dev-legacy` instead.** It is this skill frozen at its last pre-buffa revision, so
its codegen, field access and crate versions are correct for 0.7.x. Either help the user upgrade all
four crates together to the top row, or work from that skill — do not hand a 0.7 user buffa idioms,
because `MessageField`, `EnumValue`, `.as_option()` and lazy handlers do not exist there.
...
Generic layout — domain protobufs are generated by `buf generate` from a `buf.gen.yaml` and the
result is checked in. EVM additionally uses `abi/` + Abigen in a `build.rs`; Solana does not.
...
├── buf.gen.yaml                 # buffa codegen config (manual)
├── build.rs                     # EVM only: Abigen for ABI bindings
├── proto/events.proto           # Schema definitions (manual)
├── src/
│   ├── lib.rs                   # Rust module code + mod pb; (manual)
│   └── pb/                      # Generated protobuf Rust — checked in, never hand-edited
└── target/                      # Build output (gitignored)
```

**Do not hand-write generated proto Rust.** Generate it and commit the result:
...
`lazy_views=true` emits the `*LazyView` types that make lazy handlers possible (see Performance);
`unknown_fields=false` drops round-trip fidelity WASM modules do not need and keeps decode cheap.
Nothing extra goes in `[build-dependencies]` for this — `buf` does the generation, not cargo.

**`src/pb/mod.rs` is written by hand.** The buffa plugin emits one stitcher per proto package
(`myproject.v1.mod.rs`) but no top-level `mod.rs` tying them together, so you write that file:

```rust
// src/pb/mod.rs — one block per proto package, matching its `package` declaration
#[allow(clippy::all)]
pub mod myproject {
    pub mod v1 {
        include!("myproject.v1.mod.rs");
    }
}
```
...
**Lazy is selected purely by the handler signature** — no macro flag, no manifest field. `blk: &eth::BlockLazyView<'_>` is lazy, `blk: eth::Block` is eager. The `#[handlers::map]` macro reads the argument type and emits the lazy decode path when it sees a reference, so the only thing you change is the signature. The `*LazyView` types themselves come from `lazy_views=true` in `buf.gen.yaml`.

**Validation moves to access time.** An owned handler decodes the whole block up front; a lazy one surfaces a decode failure as an `Err` from the accessor that touched the field. In practice this never fires — the block reaching your module was produced and validated by the Substreams stack itself, so a malformed one means something is broken far upstream of you. Treat the error path as defensive programming rather than a case to design around, and propagate it with `?` rather than swallowing it with `unwrap_or_default()`.
...
Do **not** use the `substreams-entity-change` crate. It is deprecated and no longer supported by `graph-node`; on top of that its published versions are built against `prost` and an older `substreams` major, so they drag a second `substreams` copy into a buffa pipeline. Inline the proto instead.
...
**Missing method on ABI-generated types** ("no method named `decode`"): add `use substreams_ethereum::Event;` — the trait carries the method.
